...
#[cfg(test)]
mod tests {
    use crate::scheduler::TWaitForDependency;
    use crate::scheduler::{DependencyResult, ExecutionTaskType, Scheduler, SchedulerTask};
    use claims::assert_matches;

    use crate::mvhashmap::types::TxnIndex;
                ...
                for _ in 0..num_tasks_to_perform {
                    match tasks.pop_first().unwrap() {
                        (_, (true, txn_idx)) => {
                            ...
                            if let Ok(SchedulerTask::ValidationTask(idx, incarnation, wave)) =
                                task_res
                            {
