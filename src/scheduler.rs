...
use crate::aptos::mutex::Mutex;

use crate::mvhashmap::types::{Incarnation, TxnIndex};
...
use crate::errors::code_invariant_error;
...
//     // tasks: ConcurrentQueue<SchedulerTask>, // TODO delete
...
impl Scheduler {
    ...
    // pub fn next_task(&self) -> SchedulerTask {
    //     if self.done() {
    //         // Note this is just for testing, has a bunch of race conditions
    //         SchedulerTask::Done
    //     } else {
    //         self.done_marker.store(true, Ordering::SeqCst);
    //         let idx_to_execute = self.execution_idx.load(Ordering::Acquire);
    //         // TODO change these placeholder values
    //         // Placeholder values for Incarnation and ExecutionTaskType
    //         SchedulerTask::ExecutionTask(idx_to_execute, 0, ExecutionTaskType::Execution)
    //     }
    // }

    // pub fn finish_execution(&self) -> SchedulerTask {
    //     // fetch last value then add (post-fix operation)
    //     let mut idx_to_execute = self.execution_idx.fetch_add(1, Ordering::SeqCst);
    //     idx_to_execute += 1;
